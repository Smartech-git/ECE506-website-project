...
export async function getSteps() {
  const session = (await cookies()).get("st")?.value;
...
export async function setSteps(step: number, completedSteps: Array<number>) {
  ...
  (await cookies()).set("st", session, { expires, httpOnly: true });
  return;
}

export async function getUserTrancPin() {
  const session = (await cookies()).get("utp")?.value;
  if (!session) return
  return await decrypt(session);
}
...
export async function setUserTracPin(pin: string) {
  ...
  const session = await encrypt({ data, expires });
  (await cookies()).set("utp", session, { expires, httpOnly: true });
  return;
}
...
export async function clearRegCookies() {
  (await cookies()).set("utp", "", { expires: new Date(0) });
}

export async function getSession() {
  const session = (await cookies()).get("YS")?.value;
...
export async function createSession(data: any) {
  ...
  (await cookies()).set("YS", session, { expires, httpOnly: true });
  return;
}
...
export async function logout() {
  (await cookies()).set("YS", "", { expires: new Date(0) });
}
