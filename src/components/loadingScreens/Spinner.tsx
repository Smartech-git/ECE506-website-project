...
export default function Spinner({ className, pathClassName, strokeWidth = "5" }: { className?: string; pathClassName?: string; strokeWidth?: string }) {
  return (
    <svg width='41' height='41' viewBox='0 0 41 41' fill='none' xmlns='http://www.w3.org/2000/svg' className={`${className} animate-spin`}>
      <path
        fillRule='evenodd'
        clipRule='evenodd'
...
export const Spinner_light = ({ className }: { className?: string }) => {
  return (
    <div className={`h-[50px] w-fit ${className}`}>
      <Image alt='loading' width={500} height={500} className='h-full w-fit' src='/gifs/Jikoo logo White icon loop.gif' />
    </div>
  );
};
...
export const Spinner_green = ({ className }: { className?: string }) => {
  return (
    <div className={`h-[50px] w-fit ${className}`}>
      <Image alt='loading' width={500} height={500} className='h-full w-fit' src='/gifs/Jikoo logo Green icon loop.gif' />
    </div>
  );
};
